...
export class PermissionsGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext) {
    ...
    if (user?.role === UserRole.REQUESTER && required.module === PermissionModule.SERVICE_ORDERS && required.action === 'create') {
      return this.permissions.has(user, PermissionModule.REQUESTER_PORTAL, 'create');
    }
