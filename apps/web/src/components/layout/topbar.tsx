...
export function Topbar({ onMenu }: { onMenu(): void }) {
  ...
  return <header className="topbar">
    ...
    <div className="user-menu"><span className="avatar">{initials}</span><div><strong>{user?.name}</strong><small>{user?.role ? labels[user.role] : ''}</small></div><button className="button ghost small" onClick={logout}>Sair</button></div>
  </header>;
}
