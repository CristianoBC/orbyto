...
export function Topbar({ onMenu }: { onMenu(): void }) {
  ...
  return <header className="topbar">
    ...
    <div className="topbar-actions"><NotificationMenu /><Link className="button ghost small" href="/change-password">Alterar senha</Link><div className="user-menu"><span className="avatar">{initials}</span><div><strong>{user?.name}</strong><small>{user?.role ? labels[user.role] : ''}</small></div><button className="button ghost small" onClick={logout}>Sair</button></div></div>
  </header>;
}
