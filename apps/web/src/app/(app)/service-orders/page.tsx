...
export default function ServiceOrdersPage() {
  ...
  return <><PageHeader title="Ordens de Serviço" text="Abra e acompanhe suas solicitações." action={() => setOpen(true)} label="Nova ordem" />
    {error && <ErrorState message={error} retry={load} />}{loading ? <LoadingState /> : !items.length ? <EmptyState text="Crie sua primeira ordem de serviço para começar." /> : <div className="data-list">{items.map((item) => <article className="data-card" key={item.id}><div className="data-main"><span className="item-icon">▤</span><div><h3>{item.title}</h3><p>{item.description}</p><small>{item.category || 'Sem categoria'} · {item.system || 'Sistema não informado'} · {formatDate(item.createdAt)}</small></div></div><div className="badges"><span className={`badge priority-${item.priority?.toLowerCase()}`}>{labels[item.priority ?? 'MEDIUM']}</span><span className="badge status">{labels[item.status]}</span></div></article>)}</div>}
