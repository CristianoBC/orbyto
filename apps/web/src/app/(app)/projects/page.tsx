...
export default function ProjectsPage() {
  ...
  return <><PageHeader title="Projetos" text="Organize iniciativas, responsáveis e entregas." action={() => setOpen(true)} label="Novo projeto" />
    {error && <ErrorState message={error} retry={load} />}{loading ? <LoadingState /> : !items.length ? <EmptyState text="Crie seu primeiro projeto para organizar as entregas." /> : <div className="data-list">{items.map((item) => <Link className="data-card project-card-link" href={`/projects/${item.id}`} key={item.id} aria-label={`Abrir projeto ${item.name}`}><div className="data-main"><span className="item-icon violet">◇</span><div><h3>{item.name}</h3><p>{item.description || 'Projeto sem descrição.'}</p><small>{item.department || 'Sem departamento'} · {item.unit || 'Unidade não informada'} · {formatDate(item.createdAt)}</small></div></div><div className="project-card-end"><div className="badges"><span className={`badge priority-${item.priority.toLowerCase()}`}>{labels[item.priority]}</span><span className="badge status">{labels[item.status]}</span></div><span className="card-arrow" aria-hidden="true">→</span></div></Link>)}</div>}
