...
export default function DailyLogsPage() {
  ...
  const load = useCallback(async () => { setLoading(true); setError(''); try { setItems(await apiRequest<DailyLog[]>('/daily-logs/my')); } catch (reason) { setError(reason instanceof Error ? reason.message : 'Erro ao carregar os registros.'); } finally { setLoading(false); } }, []);
  useEffect(() => { void load(); apiRequest<Project[]>('/projects/my').then(setProjects).catch(() => setProjects([])); }, [load]);
  ...
  return <>
    <PageHeader title="Registros Diários" text="Registre e acompanhe a evolução do seu trabalho em projetos." action={showCreate} label="Novo registro" />
    ...
    {loading ? <LoadingState /> : !filtered.length ? <EmptyState text={hasFilters ? 'Nenhum registro corresponde aos filtros.' : 'Você ainda não criou registros diários.'} /> : <><p className="daily-log-count"><strong>{filtered.length}</strong> {filtered.length === 1 ? 'registro encontrado' : 'registros encontrados'}</p><DailyLogList items={filtered} onEdit={showEdit} /></>}
