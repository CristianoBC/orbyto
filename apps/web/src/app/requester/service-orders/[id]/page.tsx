...
export default function RequesterServiceOrderDetailPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(
        reason instanceof ApiError && [403, 404].includes(reason.status)
          ? "Esta solicitação não existe ou não pertence a você."
          : message(reason, "Não foi possível carregar a solicitação."),
      );
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    const success = sessionStorage.getItem("requester-success");
    if (success) {
      setNotice(success);
      sessionStorage.removeItem("requester-success");
    }
    void load();
  }, [load]);
  ...
  async function submitComment(event: FormEvent) {
    ...
    try {
      ...
      setText("");
      setComments(await apiRequest(`/comments/service-order/${id}`));
    } catch (reason) {
      setCommentError(message(reason, "Não foi possível enviar sua mensagem."));
    } finally {
      setCommenting(false);
    }
  }
  ...
  async function upload(file?: File) {
    ...
    try {
      ...
      setAttachments(await apiRequest(`/attachments/service-order/${id}`));
  ...
  if (loading)
    return (
      <div className="detail-state">
        ...
        Carregando sua solicitação...
      </div>
    );
  if (error || !order)
    return (
      <div className="detail-state detail-error">
        <strong>Não foi possível abrir esta solicitação</strong>
        <p>{error}</p>
  ...
  return (
    <div className="order-detail requester-detail">
      ...
      <header className="detail-header">
        <div>
          ...
          <h1>{order.title}</h1>
          <p>Aberta em {dateTime(order.createdAt)}</p>
        </div>
      ...
      <div className="detail-layout">
        <main className="detail-main">
          ...
          <section className="detail-card">
            ...
            <div className="info-grid">
              ...
              <Info label="Sistema ou processo" value={order.system} />
              <Info label="Unidade ou setor" value={order.unit} />
            </div>
          </section>
          <section className="detail-card">
            <div className="section-head">
              <div>
                <h2>Mensagens</h2>
                <p>Converse com a equipe responsável pelo atendimento.</p>
              </div>
            ...
            <form className="comment-form" onSubmit={submitComment}>
              <textarea
                ...
                placeholder="Escreva uma mensagem ou informação complementar..."
              />
              <div>
            ...
            <div className="comment-list">
              {comments.length ? (
                ...
                <p className="inline-empty">Ainda não há mensagens.</p>
              )}
            </div>
          </section>
          <section className="detail-card">
            <div className="section-head">
              <div>
                <h2>Anexos</h2>
                <p>Envie imagens ou documentos que ajudem no atendimento.</p>
              </div>
                      ...
                      <button
                        ...
                        onClick={() =>
                          void apiDownload(
                            ...
                            setAttachmentError(
                              message(reason, "Erro no download."),
                            ),
                          )
                        }
                      >
        ...
        <aside className="detail-sidebar">
          <section className={`summary-card${isOverdue(order) ? " service-order-overdue" : ""}`}>
            ...
            <Info label="Status" value={labels[order.status]} />
            <Info
              label="Prioridade"
            ...
            />
            <Info label={isOverdue(order) ? "Prazo vencido" : "Prazo previsto"} value={dateTime(order.dueDate)} />
            <Info label="Aberta em" value={dateTime(order.createdAt)} />
            <Info
